...
def test_scramble_is_nonempty() -> None:
    s = get_scramble("3x3")
    assert len(s.strip()) > 0


def test_scramble_has_expected_move_count() -> None:
    s = get_scramble("3x3")
    moves = count_moves(s)
...
def test_count_moves_basic() -> None:
    assert count_moves("R U R' U'") == 4
    assert count_moves("") == 0


def test_all_puzzles_return_scramble() -> None:
